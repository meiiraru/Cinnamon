...
uniform vec3 color1 = vec3(0.596078f, 0.458824f, 0.788235f);
uniform vec3 color2 = vec3(0.996078f, 0.913725f, 0.8f);
uniform vec3 color3 = vec3(0.549020f, 0.843137f, 0.968627f);
uniform vec3 color4 = vec3(0.427451f, 0.364706f, 0.854902f);
...
float color(vec2 xy) {
  return cnoise(vec3(1.5f * xy, 0.3f * time));
}

void main() {
  vec2 p = texCoords * 2.0f - 1.0f;

  vec3 xyz = vec3(p * scale, 0.0f);

  vec2 step = vec2(1.3f, 1.7f);
  float n = color(xyz.xy) * 0.5f + 0.5f;
  float n2 = color(xyz.xy + step) * 0.5f + 0.5f;
  float n3 = color(xyz.xy + step * 2.0f) * 0.5f + 0.5f;

  vec3 fc = n * color1 + (1.0f - n) * color2;
  vec3 fc2 = n2 * color3 + (1.0f - n2) * color4;

  vec3 fc3 = (1.0f - n3) * fc + n3 * fc2;

  fragColor = vec4(fc3, 1.0f);
}